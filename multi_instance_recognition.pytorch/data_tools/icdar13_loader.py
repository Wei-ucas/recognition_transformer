	...
	def get_sample(self, index):
		...
		with open(gt_path, 'r', encoding="utf-8-sig") as f:
			for line in f.readlines():
				...
				line = sline
				if self.with_script:
					line.pop(4)  # since icdar17 has script
				# Deal with transcription containing ,
				if len(line) > 5:
					word = ",".join([p.replace(' ','').replace('"', '') for p in line[4:]])
				else:
					word = line[-1].replace(' ','').replace('"','')

				temp_line = list(map(eval, line[:4]))
				x1, y1, x2, y2= map(float, temp_line)
